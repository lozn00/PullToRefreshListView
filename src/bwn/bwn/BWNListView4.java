...
/**
 * 向下那个拉拽条为什么不居中呢
 * 想通过scrollBy实现，但是头疼了，不想写了 完全不知道怎么搞，
 * 
 * @author luozheng
 * 
 */
 ...
 *
 */
public class BWNListView4 extends ListView implements OnScrollListener {
	...
	public BWNListView4(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	// @Override
	public boolean onTouchEventa(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			int paddingTop = mHeadView.getTop();

			Log.i(TAG, "headview_top:" + mHeadView.getTop());
			if (mFirstVisibleItem == 0) {// 必须点击第一个条目且 S手势是往下啦
				if (paddingTop >= mHeadHeight / 2) {
				...
				} else if (mCurrentState == State.WAIT_REFRESH || mCurrentState == State.RELREASE_REFRESH) {
	...
	/**
	 * isTouchEvent 当手还在屏幕上的时候 也就是按下的时候 如果isTouChEvent=false 那就是抬起了
	 */
	@Override
	protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY, int maxOverScrollX, int maxOverScrollY, boolean isTouchEvent) {
		maxOverScrollY = 800;

		if (isTouchEvent) {// 手指按下
			
			Log.i(TAG, "overScrollBy->"+"手指按下");
			if (mFirstVisibleItem == 0 && scrollY > 0) {// 说明拉的很长很长

				if (mCurrentState ==State.PULL_TO_REFRESH || mCurrentState==State.WAIT_REFRESH) {
					mCurrentState = State.RELREASE_REFRESH;// 第一次往下拉 释放刷新
					tvReflushState.setText("释放刷新");
					animation(mIvRefresh, false);
					Log.i(TAG, "释放刷新");
					...
					Log.i(TAG, "当前状态"+mCurrentState);
				}
			}
		} else {// 手指抬起
			Log.i(TAG, "overScrollBy->"+"自动回弹 value"+scrollY);
			
			if (mCurrentState ==State.RELREASE_REFRESH) {
				switchRefuresh();
			} 
			else{
				Log.i(TAG, "当前状态"+mCurrentState);
		...
		/*
		 * if (isTouchEvent == true && deltaY < 0 && mHeadView != null) { //
		 * 表示下拉了， // 改变headIv的高度 // 发现该高度不能超过 图片的高度 int tmpHeight = (int)
		 * (mHeadView.getTop() + Math .abs(deltaY) / mDampFactor); if (tmpHeight
		 * <= mHeadHeight) {
		 * 
		 * mHeadView.setTop(tmpHeight);
		 * 
		 * }
		 */

		// }
		Log.d(TAG, String.format("deltaY %s,scrollY %s,scrollRangeY %s,maxOverScrollY %s,isTouchEvent %s", deltaY, scrollY, scrollRangeY, maxOverScrollY, isTouchEvent));
		return super.overScrollBy(deltaX, deltaY, scrollX, scrollY, scrollRangeX, scrollRangeY, maxOverScrollX, maxOverScrollY, isTouchEvent);
		// return super.overScrollBy(deltaX, deltaY, scrollX, scrollY,
		// scrollRangeX, 500, maxOverScrollX, maxOverScrollY, isTouchEvent);
	}
}
